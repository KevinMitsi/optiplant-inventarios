...
> `docs/PHASE5.4-TRANSFERENCIAS-CIERRE.md`, `docs/PHASE5.5-LOGISTICA-CIERRE.md` y
> `docs/PHASE5.6-DASHBOARD-CIERRE.md` para el detalle de cómo se cerraron las Fases 3-6.
>
...
> `V6` (snapshot tras cerrar Fase 6 — Dashboard). El siguiente cierre (Fase 7, deuda técnica)
> debe crear `PHASE5-INVENTARIO-VENTAS-TRANSFERENCIAS-V7.md`, no editar este archivo in-place.
>
...
> Dashboard). Fase 7 → `PHASE5.7-DEUDA-TECNICA-CIERRE.md`.
...
Cierre detallado en `docs/PHASE5.6-DASHBOARD-CIERRE.md`. Resumen: records de proyección en
`domain/model` (`SalesSummary`, `ProductRotation`, `BranchComparison`), `DashboardRepositoryPort`
con `@Query` nativas sobre `sale`/`sale_item`/`inventory` (`DashboardJpaRepository`, interfaz
marcador `Repository<SaleJpaEntity, UUID>`, sin CRUD propio), `DashboardUseCase`/
`DashboardService` de solo lectura, `DashboardController` con 3 endpoints bajo
`/api/v1/organizations/{organizationId}/dashboard` (`sales-summary`, `product-rotation`,
`branch-comparison`, este último solo `ADMIN`). HU-40/41 (stock crítico / transferencias
activas) ya estaban cubiertas por `InventoryAlertController`/`TransferController`, sin trabajo
nuevo. Compila y verde: 350 tests, única falla `InventoriesApplicationTests.contextLoads`
(Docker no disponible localmente, no relacionada).

### Fase 7 — Deuda técnica — NO INICIADO
...
2. Ir directo a "Fase 7 — Deuda técnica — NO INICIADO".
3. Seguir el orden de la fase tal como está aquí; no reabrir decisiones de diseño
   salvo que aparezca un conflicto real con el esquema o el dominio.
4. Compilar y correr tests después de cada módulo, no acumular trabajo sin verificar.
5. Al cerrar Fase 7, crear `docs/PHASE5.7-DEUDA-TECNICA-CIERRE.md` y
   `docs/PHASE5-INVENTARIO-VENTAS-TRANSFERENCIAS-V7.md` (no editar este archivo in-place).
